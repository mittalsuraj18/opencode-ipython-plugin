/**
 * Jupyter WebSocket kernel client.
 *
...
const TRACE_IPC = process.env.OC_PYTHON_IPC_TRACE === "1";
const PRELUDE_INTROSPECTION_SNIPPET = "import json\nprint(json.dumps(__oc_prelude_docs__()))";
...
function throwIfAborted(signal: AbortSignal | undefined, fallbackReason: string): void {
	...
	const reason = signal.reason instanceof Error ? signal.reason : new Error(fallbackReason);
	throw reason;
}
...
export async function checkPythonKernelAvailability(cwd: string): Promise<PythonKernelAvailability> {
	if (process.env.NODE_ENV === "test" || process.env.OC_PYTHON_SKIP_CHECK === "1") {
		return { ok: true };
	}
...
export function renderKernelDisplay(content: Record<string, unknown>): {
	...
	if (data["application/x-oc-status"] !== undefined) {
		const statusData = data["application/x-oc-status"];
...
export class PythonKernel {
	...
	static async #startWithSharedGateway(
		...
		const kernel = new PythonKernel(crypto.randomUUID(), kernelId, gatewayUrl, crypto.randomUUID(), "opencode", true);

		try {
			await kernel.#connectWebSocket(startup);
			await kernel.#initializeKernelEnvironment(cwd, env, startup);
			// Prelude will be loaded by the executor/session manager
			return kernel;
		} catch (err: unknown) {
	...
	async #initializeKernelEnvironment(
		...
		const initScript = [
			"import os, sys",
			`__oc_cwd = ${JSON.stringify(cwd)}`,
			"os.chdir(__oc_cwd)",
			`__oc_env = ${JSON.stringify(envPayload)}`,
			"for __oc_key, __oc_val in __oc_env.items():\n    os.environ[__oc_key] = __oc_val",
			"if __oc_cwd not in sys.path:\n    sys.path.insert(0, __oc_cwd)",
		].join("\n");
