...
# OpenCode IPython prelude helpers
if "__oc_prelude_loaded__" not in globals():
    __oc_prelude_loaded__ = True
    ...
    def _emit_status(op: str, **data):
        ...
        display({"application/x-oc-status": {"op": op, **data}}, raw=True)
    ...
    def __oc_prelude_docs__() -> list[dict[str, str]]:
        """Return prelude helper docs for templating. Discovers functions by _oc_category attribute."""
        helpers: list[dict[str, str]] = []
        for name, obj in globals().items():
            if not callable(obj) or not hasattr(obj, "_oc_category"):
                continue
